...
static StageResult history_index_stage(db::TransactionManager& txn, const std::filesystem::path& etl_path, bool storage) {
...
StageResult history_index_unwind(db::TransactionManager& txn, const std::filesystem::path& etl_path, uint64_t unwind_to,
                                 bool storage) {
...
StageResult history_index_prune(db::TransactionManager& txn, const std::filesystem::path& etl_path, uint64_t prune_from,
                                bool storage) {
    db::MapConfig index_config = storage ? db::table::kStorageHistory : db::table::kAccountHistory;
    const char* stage_key = storage ? db::stages::kStorageHistoryIndexKey : db::stages::kAccountHistoryIndexKey;
...
StageResult stage_account_history(db::TransactionManager& txn, const std::filesystem::path& etl_path, uint64_t) {
    return history_index_stage(txn, etl_path, false);
}
StageResult stage_storage_history(db::TransactionManager& txn, const std::filesystem::path& etl_path, uint64_t) {
