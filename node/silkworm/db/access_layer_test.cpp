...
namespace db {
    ...
    TEST_CASE("read_stages") {
        ...
        CHECK_THROWS(stages::read_stage_unwind_progress(txn, "NonExistentStage"));

        // Not valued stage should return 0
        CHECK(stages::read_stage_progress(txn, stages::kBlockBodiesKey) == 0);
        CHECK(stages::read_stage_unwind_progress(txn, stages::kBlockBodiesKey) == 0);
        ...
        CHECK_NOTHROW(stages::write_stage_unwind_progress(txn, stages::kBlockBodiesKey, expected_block_num));
        CHECK_NOTHROW(block_num = stages::read_stage_progress(txn, stages::kBlockBodiesKey));
        CHECK(block_num == expected_block_num);
        CHECK_NOTHROW(block_num = stages::read_stage_unwind_progress(txn, stages::kBlockBodiesKey));
        CHECK(block_num == expected_block_num);
        CHECK_NOTHROW(stages::write_stage_unwind_progress(txn, stages::kBlockBodiesKey));
        CHECK(!stages::read_stage_unwind_progress(txn, stages::kBlockBodiesKey));
